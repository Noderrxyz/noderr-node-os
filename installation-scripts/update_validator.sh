...
#
# Usage (from inside the Validator LXC — recommended):
#   curl -fsSL https://raw.githubusercontent.com/Noderrxyz/noderr-node-os/master/installation-scripts/update_validator.sh -o /tmp/update_validator.sh && bash /tmp/update_validator.sh
#
# Usage (from the Proxmox host shell — replace CT_ID with your Validator CT ID):
#   pct exec <CT_ID> -- bash -c "curl -fsSL https://raw.githubusercontent.com/Noderrxyz/noderr-node-os/master/installation-scripts/update_validator.sh -o /tmp/update_validator.sh && bash /tmp/update_validator.sh"
#
