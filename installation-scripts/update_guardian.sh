...
readonly IMAGE_URL="${R2_PUBLIC_URL}/guardian/guardian-latest.tar.gz"
readonly TMP_IMAGE="/tmp/noderr-guardian-latest.tar.gz"
...
echo "=========================================================="
echo "  Noderr Guardian Node - Image Update"
echo "  $(date)"
echo "=========================================================="
...
log "Downloading new guardian image from R2..."
log "  URL: ${IMAGE_URL}"
...
CHECKSUM_URL="${R2_PUBLIC_URL}/guardian/guardian-latest.tar.gz.sha256"
if curl -fsSL "${CHECKSUM_URL}" -o /tmp/guardian-latest.tar.gz.sha256 2>/dev/null; then
    # R2 checksum file format: "<hash>  <filename>"
    EXPECTED_HASH=$(awk '{print $1}' /tmp/guardian-latest.tar.gz.sha256)
...
docker images noderr-guardian:latest --format "  Repository: {{.Repository}}  Tag: {{.Tag}}  Size: {{.Size}}  Created: {{.CreatedAt}}" | tee -a "$LOG_FILE"
...
# ── Cleanup ───────────────────────────────────────────────────────────────────
rm -f "${TMP_IMAGE}" /tmp/guardian-latest.tar.gz.sha256
