...
describe('Protocol Config - Staking', () => {
  describe('STAKING_REQUIREMENTS', () => {
    it('should have correct values matching smart contracts', () => {
      ...
      expect(STAKING_REQUIREMENTS[NodeTier.VALIDATOR]).toBe(ethers.parseUnits('50000', 18));
      expect(STAKING_REQUIREMENTS[NodeTier.GUARDIAN]).toBe(ethers.parseUnits('100000', 18));
      expect(STAKING_REQUIREMENTS[NodeTier.ORACLE]).toBe(ethers.parseUnits('500000', 18));
    });
  ...
  describe('getStakingRequirement', () => {
    it('should return correct requirement for enum value', () => {
      expect(getStakingRequirement(NodeTier.ORACLE)).toBe(ethers.parseUnits('500000', 18));
      expect(getStakingRequirement(NodeTier.GUARDIAN)).toBe(ethers.parseUnits('100000', 18));
      expect(getStakingRequirement(NodeTier.VALIDATOR)).toBe(ethers.parseUnits('50000', 18));
    ...
    it('should return correct requirement for string name', () => {
      expect(getStakingRequirement('oracle')).toBe(ethers.parseUnits('500000', 18));
      expect(getStakingRequirement('guardian')).toBe(ethers.parseUnits('100000', 18));
      expect(getStakingRequirement('validator')).toBe(ethers.parseUnits('50000', 18));
  ...
  describe('formatStakingRequirement', () => {
    it('should format requirements as human-readable strings', () => {
      expect(formatStakingRequirement(NodeTier.ORACLE)).toBe('500000.0 NODR');
      expect(formatStakingRequirement(NodeTier.GUARDIAN)).toBe('100000.0 NODR');
      expect(formatStakingRequirement(NodeTier.VALIDATOR)).toBe('50000.0 NODR');
      expect(formatStakingRequirement(NodeTier.MICRO)).toBe('0.0 NODR');
    });

    it('should format requirements from string names', () => {
      expect(formatStakingRequirement('oracle')).toBe('500000.0 NODR');
      expect(formatStakingRequirement('guardian')).toBe('100000.0 NODR');
    });
  });

  describe('hasSufficientStake', () => {
    it('should return true when balance meets requirement', () => {
      const balance = ethers.parseUnits('500000', 18);
    ...
    it('should return false when balance is insufficient', () => {
      const balance = ethers.parseUnits('49999', 18);
