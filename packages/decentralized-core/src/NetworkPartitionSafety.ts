...
export class NetworkPartitionSafety extends EventEmitter {
  ...
  private readonly ELECTION_TIMEOUT_MAX = 6000; // 6 seconds
  private readonly FAILURE_THRESHOLD = 3; // missed heartbeats before suspected
  private readonly PARTITION_THRESHOLD = 5; // missed heartbeats before partitioned
  ...
  private simulateHeartbeatResponse(nodeId: string, heartbeat: HeartbeatMessage): void {
    const node = this.nodes.get(nodeId)!;
  ...
  private checkNodeHealth(): void {
    ...
    const suspectedNodes = new Set<string>();
  ...
  detectPartition(): PartitionDetection {
    ...
    let action: PartitionDetection['action'] = 'continue';
    ...
    const detection: PartitionDetection = {
      timestamp: new Date(),
  ...
  private handlePartition(partition: PartitionDetection): void {
  ...
  receiveHeartbeat(heartbeat: HeartbeatMessage): void {
    ...
    if (heartbeat.commitIndex > this.consensusState.commitIndex) {
      this.consensusState.commitIndex = Math.min(
