...
export interface ConsensusConfig {
  ...
  // Signal validity window (milliseconds)
  signalValidityWindow: number;
  // Weight reputation in consensus
 ...
 */
export class SignalConsensus extends EventEmitter {
  private config: ConsensusConfig;
  private logger: winston.Logger;
  private activeSignals: Map<string, TradingSignal[]> = new Map();
  private consensusHistory: ConsensusResult[] = [];
  private nodeReputations: Map<string, NodeReputation> = new Map();
   ...
   */
  async submitSignal(signal: TradingSignal): Promise<void> {
  ...
  private checkConsensus(): void {
    ...
    for (const [key, signals] of this.activeSignals) {
      ...
      const validSignals = signals.filter(s => 
        now - s.timestamp.getTime() <= this.config.signalValidityWindow
      );
   ...
   */
  private checkConsensusForSymbol(key: string): void {
   ...
   */
  private calculateSignalWeight(signal: TradingSignal): number {
   ...
   */
  private async verifySignature(signal: TradingSignal): Promise<boolean> {
    if (!signal.signature) {
      return false;
    }
    ...
    try {
      ...
      const recoveredAddress = ethers.verifyMessage(messageHash, signal.signature);
