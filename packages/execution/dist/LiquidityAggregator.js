...
class LiquidityAggregator extends events_1.default {
    logger;
    state;
    cache;
    // [NODERR_EXEC_OPTIMIZATION_STAGE_3_LIQUIDITY]: Enhanced cache system
    cexCache; // Fast cache for CEX data
    dexCache; // Slower cache for DEX data
    cacheConfig;
    updateInterval;
    warmupInterval;
    reconnectAttempts;
    maxConcurrentVenues = 10; // Concurrency limit
    batchProcessor;
    constructor(logger, exchanges, cacheConfig) {
        super();
        this.logger = logger;
        ...
        this.cache = new node_cache_1.default({ stdTTL: this.cacheConfig.defaultTTL / 1000, checkperiod: 0.5 });
        this.cexCache = new node_cache_1.default({
            stdTTL: this.cacheConfig.cexTTL / 1000,
            checkperiod: 0.1,
            maxKeys: this.cacheConfig.maxCacheSize
        });
        this.dexCache = new node_cache_1.default({
     ...
     */
    async getAggregatedLiquidity(symbol) {
        ...
        try {
            ...
            throw new types_1.ExecutionError(types_1.ExecutionErrorCode.EXCHANGE_ERROR, 'Failed to aggregate liquidity');
        }
    }
    // [NODERR_EXEC_OPTIMIZATION_STAGE_3_LIQUIDITY]: Smart cache system with venue-specific TTLs
    getFromSmartCache(cacheKey, symbol) {
        // Try CEX cache first (fastest)
        let cached = this.cexCache.get(cacheKey);
        if (cached) {
        ...
        // Try DEX cache
        cached = this.dexCache.get(cacheKey);
    ...
    storeInSmartCache(cacheKey, snapshot, symbol) {
    ...
    getDynamicInvalidationThreshold(symbol) {
    ...
    updatePriceTracking(symbol, exchangeLiquidity) {
    ...
    async getExchangeLiquidityParallel(symbol) {
        ...
        const promises = exchanges.map(exchange => this.getLiquidityWithConcurrencyLimit(symbol, exchange));
        // [NODERR_EXEC_OPTIMIZATION_STAGE_3_LIQUIDITY]: Use Promise.allSettled for resilient parallel execution
        const results = await Promise.allSettled(promises);
        const exchangeLiquidity = [];
        const errors = [];
        ...
        if (exchangeLiquidity.length === 0) {
            ...
            throw new types_1.ExecutionError(types_1.ExecutionErrorCode.ALL_VENUES_FAILED, `All ${exchanges.length} venues failed for ${symbol}: ${errors.join('; ')}`);
        }
    ...
    async getLiquidityWithConcurrencyLimit(symbol, exchange) {
    ...
    queueBatchRequest(exchange, symbol) {
        ...
        for (const [exchange, queue] of this.state.requestQueue) {
            ...
            if (availableSlots > 0) {
                ...
                for (const request of requestsToProcess) {
                    try {
                        await this.getLiquidityWithConcurrencyLimit(request.symbol, this.state.exchanges.get(exchange));
                    }
                    catch (error) {
    ...
    updateVenueLatency(exchangeId, latency) {
    ...
    scheduleRetryWithBackoff(symbol, exchangeId) {
    ...
    initializeConnections() {
        for (const exchange of this.state.exchanges.values()) {
            if (exchange.capabilities.includes('WEBSOCKET_FEED')) {
    ...
    handleOrderBookUpdate(exchangeId, message) {
    ...
    handleTradeUpdate(exchangeId, message) {
    ...
    handleTickerUpdate(exchangeId, message) {
    ...
    async getExchangeLiquidity(symbol) {
     ...
     */
    async getExchangeLiquidityData(symbol, exchangeId, orderBook) {
    ...
    aggregateOrderBooks(exchangeLiquidity) {
    ...
    calculateBestPrices(exchangeLiquidity) {
        let bestBid = { price: 0, quantity: 0 };
        let bestAsk = { price: Infinity, quantity: 0 };
        for (const exchange of exchangeLiquidity) {
    ...
    subscribeToExchange(exchangeId, symbols) {
    ...
    unsubscribeFromExchange(exchangeId, symbols) {
        const ws = this.state.connections.get(exchangeId);
        if (!ws || ws.readyState !== ws_1.default.OPEN) {
            return;
        }
    ...
    getLastTradePrice(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`);
        return trades && trades.length > 0 ? trades[0].price : 0;
    }
    calculate24hVolume(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`) || [];
    ...
    calculate24hHigh(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`) || [];
        const cutoff = Date.now() - 24 * 60 * 60 * 1000;
        const prices = trades
            .filter(t => t.timestamp > cutoff)
    ...
    calculate24hLow(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`) || [];
        const cutoff = Date.now() - 24 * 60 * 60 * 1000;
        const prices = trades
            .filter(t => t.timestamp > cutoff)
    ...
    calculate24hVWAP(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`) || [];
        const cutoff = Date.now() - 24 * 60 * 60 * 1000;
    ...
    count24hTrades(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`) || [];
        const cutoff = Date.now() - 24 * 60 * 60 * 1000;
        return trades.filter(t => t.timestamp > cutoff).length;
    }
    calculateVolatility(symbol, exchangeId) {
        const trades = this.state.trades.get(`${symbol}-${exchangeId}`) || [];
        if (trades.length < 20)
            return 0;
        // Calculate returns
    ...
    aggregateMarketData(exchanges) {
        ...
        // Find best bid/ask
        let bestBid = { price: 0, quantity: 0, exchange: '', timestamp: Date.now() };
        let bestAsk = { price: Infinity, quantity: 0, exchange: '', timestamp: Date.now() };
    ...
    createMockTrade(symbol, exchangeId) {
        return {
            id: `mock-${Date.now()}`,
            symbol,
            price: 0,
