...
const KellyCriterion_1 = require("./KellyCriterion");
...
describe('KellyCriterion', () => {
    ...
    beforeEach(() => {
        ...
        kelly = new KellyCriterion_1.KellyCriterion(config);
    });
    describe('constructor', () => {
        it('should create a new instance with valid config', () => {
            expect(kelly).toBeInstanceOf(KellyCriterion_1.KellyCriterion);
        });
        it('should throw error with invalid confidence level', () => {
            expect(() => {
                new KellyCriterion_1.KellyCriterion({ confidenceLevel: 0, minTrades: 30 });
            }).toThrow(phoenix_types_1.MLError);
            expect(() => {
                new KellyCriterion_1.KellyCriterion({ confidenceLevel: 1.5, minTrades: 30 });
            }).toThrow(phoenix_types_1.MLError);
        });
        it('should throw error with invalid minTrades', () => {
            expect(() => {
                new KellyCriterion_1.KellyCriterion({ confidenceLevel: 0.25, minTrades: 0 });
            }).toThrow(phoenix_types_1.MLError);
        });
    });
    describe('calculatePositionSize', () => {
        ...
        it('should throw error with invalid symbol', async () => {
            await expect(kelly.calculatePositionSize('', 0.7, 0.25)).rejects.toThrow(phoenix_types_1.MLError);
        });
        it('should throw error with invalid confidence', async () => {
            await expect(kelly.calculatePositionSize('BTC/USD', -0.1, 0.25)).rejects.toThrow(phoenix_types_1.MLError);
            await expect(kelly.calculatePositionSize('BTC/USD', 1.5, 0.25)).rejects.toThrow(phoenix_types_1.MLError);
        });
        it('should throw error with invalid max position size', async () => {
            await expect(kelly.calculatePositionSize('BTC/USD', 0.7, 0)).rejects.toThrow(phoenix_types_1.MLError);
            await expect(kelly.calculatePositionSize('BTC/USD', 0.7, 1.5)).rejects.toThrow(phoenix_types_1.MLError);
        });
...
//# sourceMappingURL=KellyCriterion.test.js.map