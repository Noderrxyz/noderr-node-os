...
const logger = new Logger('PortfolioSentinel');
...
interface RebalanceTrigger {
  id: string;
  type: 'THRESHOLD' | 'SCHEDULE' | 'EVENT' | 'RISK';
  condition: string;
  threshold: number;
  action: 'REBALANCE' | 'REDUCE' | 'FREEZE' | 'ALERT';
...
export class PortfolioSentinel extends EventEmitter {
  private logger: ReturnType<typeof createLogger>;
  private portfolioState: PortfolioState;
  private constraints: PortfolioConstraints;
  private rebalanceTriggers: Map<string, RebalanceTrigger>;
  private capitalFlows: CapitalFlow[];
  ...
  private initializeConstraints(): PortfolioConstraints {
    return {
      maxDrawdown: 0.20, // 20%
      maxLeverage: 2.0,
  ...
  private registerTrigger(trigger: RebalanceTrigger): void {
  ...
  public async updatePrice(symbol: string, price: number): Promise<void> {
    const position = this.portfolioState.positions.get(symbol);
    if (!position) return;
  ...
  private updatePortfolioState(): void {
    ...
    if (this.highWaterMark > 0) {
      ...
      this.portfolioState.metrics.maxDrawdown = Math.max(
        this.portfolioState.metrics.maxDrawdown,
        this.portfolioState.metrics.currentDrawdown
      );
    }
  ...
  private evaluateTrigger(trigger: RebalanceTrigger): boolean {
    switch (trigger.id) {
  ...
  private handleTrigger(trigger: RebalanceTrigger): void {
  ...
  private handleConstraintViolation(constraint: string, value: number, context?: string): void {
    this.logger.error('Constraint violation', {
      constraint,
      value,
      limit: (this.constraints as any)[constraint],
      context
    });
  ...
  public freezeCapital(reason: string): void {
    if (this.frozen) return;
    ...
    const action: EmergencyAction = {
      ...
      reason,
      severity: 'CRITICAL',
      targetStrategies: Array.from(new Set(
        Array.from(this.portfolioState.positions.values()).map(p => p.strategy)
      )),
      status: 'EXECUTING'
    };
  ...
  private reduceExposure(priority: string): void {
  ...
  private requestRebalance(trigger: RebalanceTrigger): void {
  ...
  private sendAlert(trigger: RebalanceTrigger): void {
    this.emit('alert', {
  ...
  public recordCapitalFlow(flow: Omit<CapitalFlow, 'id' | 'timestamp' | 'impact'>): void {
  ...
  public updateConstraints(newConstraints: Partial<PortfolioConstraints>): void {
    this.constraints = {
      ...this.constraints,
      ...newConstraints
    };
  ...
  public getEmergencyActions(): EmergencyAction[] {
